...
step4to5 <- function(data, variable.name) {
  ...
  # Gamma distribution
  f<- as.formula(paste(variable.name, " ~ .", sep = ""))
  modsGamma <- get.models(dredge(
    glm(f,
        family = Gamma(link = "log"), data = data, na.action = "na.fail")),
  ...
  f1 <- as.formula(paste( "log(", variable.name, ")",  " ~ .", sep = ""))
  modsGaussianLog <- get.models(dredge(
    glm(f1,
        data = data, na.action = "na.fail")),
    subset = cumsum(weight) <= 0.95)[1]
  # summarize the 3 models in one list
  combine <- list(modsGamma, modsGaussian, modsGaussianLog)
  names(combine) <- c("Gamma", "Gaussian", "GaussianLog")
  
  # print QQPlots for the 3 models to determine which is the best fit
  op <- par(mfrow = c(3,1))
  qqnorm(resid(modsGamma[[1]]), main = "Gamma QQPlot")
  qqline(resid(modsGamma[[1]]))
  qqnorm(resid(modsGaussian[[1]]), main = "Gaussian QQPlot")
  qqline(resid(modsGaussian[[1]]))
  qqnorm(resid(modsGaussianLog[[1]]), main = "GaussianLog QQPlot")
  qqline(resid(modsGaussianLog[[1]]))
  # return the list of model summaries
  return(combine)
}

step6 <- function(bestModel) { # best Model is the call , data is the dataframe
...
z
```
The best-fitting model according to the dredge() function is: `r grav_mois ~ GrazeTime + 1`. This has an AIC of 265, which is quite high. However, the QQplot of the residuals still doesn't look good, so this type of model still does not fit this dataset well. The summary indicates that almost all GrazeTimes are significant.

```{r}
(step4to5(data = cn, variable.name = cn$grav_mois))
...
For this dataset, a log transformation suitably improves the data to run a GLM. The best fit model shows that dissolved organic nitrogen is strongly related to dissolved organic carbon, and GrazeTime is also a significant factor, along with a Block effect
                 
```{r}
# fit a Gaussian (normal) distribution with a log transformation
(get.models(dredge(glm(log(NPOC_mgkgdrysoil) ~ ., data = cn, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(NPOC_mgkgdrysoil) ~ Block + DON_mgkgdrysoil + 
                   GrazeTime + Treatment + 1, data = cn, na.action = "na.fail")))
qqline(resid(glm(formula = log(NPOC_mgkgdrysoil) ~ Block + DON_mgkgdrysoil + 
                   GrazeTime + Treatment + 1, data = cn, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(NPOC_mgkgdrysoil) ~ Block + DON_mgkgdrysoil + 
              GrazeTime + Treatment + 1, data = cn, na.action = "na.fail"))
...
plot(cn$Treatment, resid(lm(DON_mgkgdrysoil ~ Treatment * GrazeTime, data = cn)), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
For this dataset, non-transformed data had the smallest AIC, and the best fit model determined that mineral N and NPOC were the strongest determinants of DON. 
```{r}
# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(DON_mgkgdrysoil ~ ., data = cn, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = DON_mgkgdrysoil ~ mineralN_mgkgdrysoil + NPOC_mgkgdrysoil + 
                   1, data = cn, na.action = "na.fail")))
qqline(resid(glm(formula = DON_mgkgdrysoil ~ mineralN_mgkgdrysoil + NPOC_mgkgdrysoil + 
                   1, data = cn, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = DON_mgkgdrysoil ~ mineralN_mgkgdrysoil + NPOC_mgkgdrysoil + 
              1, data = cn, na.action = "na.fail"))
...
plot(cn$Treatment, resid(lm(NO3_mgkgdrysoil ~ Treatment * GrazeTime, data = cn)), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
Based on solely an AIC test, it seems that the Gaussian distribution with a log transformation was the most appropriate action for this data, but the QQplot of the residuals of the selected model parameters still does not fit well. The selected model parameters were DON and NPOC, which illustrates that all biogeochemical parameters are more related to either other than GrazeTime or Treatment.
```{r}
...
# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(log(NO3_mgkgdrysoil) ~ ., data = cn, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(NO3_mgkgdrysoil) ~ DON_mgkgdrysoil + NPOC_mgkgdrysoil + 
                   1, data = cn, na.action = "na.fail")))
qqline(resid(glm(formula = log(NO3_mgkgdrysoil) ~ DON_mgkgdrysoil + NPOC_mgkgdrysoil + 
                   1, data = cn, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(NO3_mgkgdrysoil) ~ DON_mgkgdrysoil + NPOC_mgkgdrysoil + 
              1, data = cn, na.action = "na.fail"))
...
A log-transformed Gaussian distribution fit this data most appropriately, and the selected model parameters show significance with Gravimetric moisture, mineral N, nitrate, and NPOC.
```{r}

# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(log(NH4_mgkgdrysoil) ~ ., data = cn, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(NH4_mgkgdrysoil) ~ grav_mois + mineralN_mgkgdrysoil + 
                   NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, data = cn, na.action = "na.fail")))
qqline(resid(glm(formula = log(NH4_mgkgdrysoil) ~ grav_mois + mineralN_mgkgdrysoil + 
                   NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, data = cn, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(NH4_mgkgdrysoil) ~ grav_mois + mineralN_mgkgdrysoil + 
              NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, data = cn, na.action = "na.fail"))
...
plot(cn$Treatment, resid(lm(mineralN_mgkgdrysoil ~ Treatment * GrazeTime, data = cn)), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
This data also seems to fit a log-tranformed Gaussian well. The summary of the best fit model parameters shows significance with DON, ammonium, nitrate, and NPOC.
```{r}

# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(log(mineralN_mgkgdrysoil) ~ ., data = cn, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(mineralN_mgkgdrysoil) ~ DON_mgkgdrysoil + grav_mois + 
                   NH4_mgkgdrysoil + NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, 
                 data = cn, na.action = "na.fail")))
qqline(resid(glm(formula = log(mineralN_mgkgdrysoil) ~ DON_mgkgdrysoil + grav_mois + 
                   NH4_mgkgdrysoil + NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, 
                 data = cn, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(mineralN_mgkgdrysoil) ~ DON_mgkgdrysoil + grav_mois + 
              NH4_mgkgdrysoil + NO3_mgkgdrysoil + NPOC_mgkgdrysoil + 1, 
            data = cn, na.action = "na.fail"))
...
plot(cn$Treatment, resid(lm(Enzyme_nm_g_hr ~ Treatment * GrazeTime, data = enz[enz$Substrate %in% "AG",])), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
The GLM model shows that the best fitting explanatory variables are Block and Treatment. The QQplot still is not fitting well, but the summary shows that there are significant differences between Block 4, and between the LO and NO treatments.

```{r}
# massage the data to make GLM syntax happy
x <- enz[enz$Substrate %in% "AG",]
x$Substrate <- NULL
x$Enzyme_nm_g_hr[x$Enzyme_nm_g_hr == 0] <- 0.000000001
# fit a Gaussian (normal) distribution 
(get.models(dredge(
  glm(log(Enzyme_nm_g_hr) ~ ., data = x ,  na.action = "na.fail")),
  subset = cumsum(weight) <= 0.95))[1]

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(Enzyme_nm_g_hr) ~ Block + Treatment + 1, data = x, 
                 na.action = "na.fail")))
qqline(resid(glm(formula = log(Enzyme_nm_g_hr) ~ Block + Treatment + 1, data = x, 
                 na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(Enzyme_nm_g_hr) ~ Block + Treatment + 1, data = x, 
            na.action = "na.fail"))
...
plot(cn$Treatment, resid(lm(Enzyme_nm_g_hr ~ Treatment * GrazeTime, data = enz[enz$Substrate %in% "BG",])), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
This data looks very similar to AG, where the model selects Plot as the best fitting explanatory variable. However, several outliers still skew the QQplot.
```{r}
# massage the data to make GLM syntax happy
x <- enz[enz$Substrate %in% "BG",]
x$Substrate <- NULL
x$Enzyme_nm_g_hr[x$Enzyme_nm_g_hr == 0] <- 0.000000001
# fit a Gaussian (normal) distribution 
(get.models(dredge(
...
# check the normality of the residuals with a QQplot
qqnorm(resid(glm(formula = log(Enzyme_nm_g_hr) ~ Plot + 1, data = x, na.action = "na.fail")))
qqline(resid(glm(formula = log(Enzyme_nm_g_hr) ~ Plot + 1, data = x, na.action = "na.fail")))

# summarize the best fit model
summary(glm(formula = log(Enzyme_nm_g_hr) ~ Plot + 1, data = x, na.action = "na.fail"))
...
plot(veg$Treatment, resid(lm(biomass_kg_plot ~ Treatment * GrazeTime, data = veg)), xlab = "Treatment", ylab = "Residuals",
     main = "Residuals vs. explan. var.")
```
The best fitting model based on the dredge() function is including parameters of Grazetime, Treatment, and their interaction. There is significance with GrazeTime, and with GrazeTime/Treatment interactions.

```{r}
# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(log(biomass_kg_plot) ~ ., data = veg, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(log(biomass_kg_plot) ~ GrazeTime + Treatment + 
                   GrazeTime:Treatment + 1, data = veg, na.action = "na.fail")))
qqline(resid(glm(log(biomass_kg_plot) ~ GrazeTime + Treatment + 
                   GrazeTime:Treatment + 1, data = veg, na.action = "na.fail")))

# summarize the best fit model
summary(glm(log(biomass_kg_plot) ~ GrazeTime + Treatment + 
              GrazeTime:Treatment + 1, data = veg, na.action = "na.fail"))
...
```
This is a narrower dataset, as there are less datapoints. The best fit model was a Gaussian distribution without a log transformation. The two treatments are significant to each other.
```{r}
# fit a Gaussian (normal) distribution 
(get.models(dredge(glm(forage_ut ~ Treatment, data = for.ut18, na.action = "na.fail")),
            subset = cumsum(weight) <= 0.95))

# check the normality of the residuals with a QQplot
qqnorm(resid(glm(forage_ut ~ Treatment + 1, data = for.ut18, na.action = "na.fail")))
qqline(resid(glm(forage_ut ~ Treatment + 1, data = for.ut18, na.action = "na.fail")))

# summarize the best fit model
summary(glm(forage_ut ~ Treatment + 1, data = for.ut18, na.action = "na.fail"))
