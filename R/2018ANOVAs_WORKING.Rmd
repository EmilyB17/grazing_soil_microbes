...
step4to5 <- function(data, variable.name) {
  ...
  f2 <- as.formula(paste( "log1p(", variable.name, ")",  " ~ .", sep = ""))
...
step4to5zeroinfl <- function(data, variable.name) {
  ...
  modsGaussianLog <- get.models(dredge(
    ...
    subset = cumsum(weight) <= 0.95)[1]
  # summarize the 3 models in one list
  combine <- list(modsGaussian, modsGaussianLog)
  names(combine) <- c("Gaussian", "GaussianLog")
...
The best fitting model is a log-tranformed Gaussian, although the QQplot shows that all of the models are skewed by outliers. The best explanatory variables are Block, which indiates that there is a strong block effect. The model significance plot confirms this by illustrating that Block 1 (Intercept) is strongly significant to the other blocks. 

This suggests that even if there is a treatment or time effect, the block effect is so strong that it is masking the other potential interactions.
...
This data is TOO zero-inflated to run properly on any sort of GLM.
