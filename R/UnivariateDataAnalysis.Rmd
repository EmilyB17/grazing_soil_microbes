---
title: "2018 & 2017 Data Analysis - Univariate"
author: "Emily Bean"
...
z <- step6(bestModel = glm(formula = log1p(grav_mois) ~ GrazeTime + NO3_mgkgdrysoil + 
...
There is one influential measure, and the linear regression assumptions are met except for the fitted residuals, which show vertical patterns. The best fitting model based on AIC is the log-tranformed Gaussian distribution, which shows that the explanatory variables are GrazeTime and Treatment. However, the Gaussian All model illustrates a strong block effect.
...
y
# make sure to change "data" to the dataframe
z <- step6(bestModel = glm(formula = log1p(NPOC_mgkgdrysoil) ~ GrazeTime + Treatment + 
    1, data = cn, na.action = "na.fail"))
z
```
...
Removing block 4 makes the data fit the assumptions of the linear regression, so we can quickly run a repeated-measures ANOVA, and once GrazeTime is considered a repeated measure the Treatment loses significance.

```{r}
veg$reading_rpm <- NULL
veg <- veg[veg$Block %in% 1:3,]

x <- step1to3(data = veg, variable.name = "biomass_kg_plot")
y <- step4to5(data = veg, variable.name = "biomass_kg_plot")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel = glm(formula = log1p(biomass_kg_plot) ~ Treatment + 1, 
                           data = veg, na.action = "na.fail"))
z

# since log-transfomed meets linear regression assumptions, do ANOVA
summary(aov(log1p(biomass_kg_plot) ~ Treatment*GrazeTime + Block + Error(biomass_kg_plot / GrazeTime), data = veg))
...
*1/25/19 update: to minimize psuedoreplication, the 5 replicates from each plot are averaged for a total of 1 averaged sample per plot, similar to 2018 data*
...
```{r}
x <- step1to3(data = grav17.avg, variable.name = "grav_mois")
y <- step4to5(data = grav17.avg, variable.name = "grav_mois")
y
# make sure to change "data" to the dataframe
...
grav17.avg <- grav17.avg[grav17.avg$Block %in% 1:3,]
# since this data fits assumptions of linear regression once log-transformed, do repeated-measures ANOVA
summary(aov(log1p(grav_mois) ~ Treatment * GrazeTime + Block + Error(grav_mois / GrazeTime), data = grav17.avg))
...
ph17.avg <- data.frame(summarise(group_by(.data = ph17, Block, Treatment, GrazeTime),
                                 pH = mean(pH, na.rm = TRUE)))

x <- step1to3(data = ph17.avg, variable.name = "pH")
ph17.avg <- ph17.avg[ph17.avg$Block %in% 1:3,]
y <- step4to5(data = ph17.avg, variable.name = "pH")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(pH) ~ Block + 1, family = Gamma(link = "log"), 
                          data = ph17.avg, na.action = "na.fail")
)
z
...
```{r}
x <- step1to3(data = cn17.avg, variable.name = "NH4_mgkgdrysoil")
y <- step4to5(data = cn17.avg, variable.name = "NH4_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(NH4_mgkgdrysoil) ~ GrazeTime+ 1, data = cn17.avg, 
                          na.action = "na.fail")
)
z

# try repeated-measures ANOVA since data normalizes decently with log transformation
summary(aov(log1p(NH4_mgkgdrysoil) ~ Treatment*GrazeTime + Block + Error(log1p(NH4_mgkgdrysoil) / GrazeTime), data = cn17.avg))
...
```{r}
x <- step1to3(data = cn17.avg, variable.name = "mineralN_mgkgdrysoil")
y <- step4to5(data = cn17.avg, variable.name = "mineralN_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(mineralN_mgkgdrysoil) ~ GrazeTime + 1, data = cn17.avg, 
                          na.action = "na.fail")
)
z

# try repeated-measures ANOVA
summary(aov(log1p(mineralN_mgkgdrysoil) ~ Treatment*GrazeTime + Block + Error(log1p(mineralN_mgkgdrysoil) / GrazeTime), data = cn17.avg))
...
```{r}
x <- step1to3(data = cn17.avg, variable.name = "NPOC_mgkgdrysoil")
y <- step4to5zeroinfl(data = cn17.avg, variable.name = "NPOC_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(NPOC_mgkgdrysoil) ~ GrazeTime +  Treatment + 
                            1, data = cn17.avg, na.action = "na.fail")
)
z
...
```{r}
x <- step1to3(data = cn17.avg, variable.name = "DON_mgkgdrysoil")
y <- step4to5(data = cn17.avg, variable.name = "DON_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(DON_mgkgdrysoil) ~ GrazeTime + 1, data = cn17.avg, 
                          na.action = "na.fail")
)
z

# repeated-measures ANOVA
summary(aov(log1p(DON_mgkgdrysoil) ~ Treatment*GrazeTime + Block + Error(log1p(DON_mgkgdrysoil) / GrazeTime), data = cn17.avg))
...
mbc.avg <- data.frame(summarise(group_by(.data = mbc17, Block, Treatment, GrazeTime),
                                 MBC_mgkgdrysoil = mean(MBC_mgkgdrysoil, na.rm = TRUE),
                                MBN_mgkgdrysoil = mean(MBN_mgkgdrysoil, na.rm = TRUE)))





x <- step1to3(data = mbc.avg, variable.name = "MBC_mgkgdrysoil")
mbc.avg <- mbc.avg[mbc.avg$Block %in% 1:3,]
y <- step4to5zeroinfl(data = mbc.avg, variable.name = "MBC_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(MBC_mgkgdrysoil) ~ GrazeTime + 1, data = mbc.avg, 
                          na.action = "na.fail")
)
z

# repeated-measures ANOVA
summary(aov(log1p(MBC_mgkgdrysoil) ~ Treatment * GrazeTime + Block + Error(log1p(MBC_mgkgdrysoil) / GrazeTime), data = mbc.avg))
...
After removing block 4, a repeated-measures ANOVA indicates that Treatment is significant. However, a post-hoc paired bonferroni test does not indicate that any treatments are significant to each other at a level of 0.05.

```{r}
x <- step1to3(data = mbc.avg, variable.name = "MBN_mgkgdrysoil")
y <- step4to5zeroinfl(data = mbc.avg, variable.name = "MBN_mgkgdrysoil")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel =glm(formula = log1p(MBN_mgkgdrysoil) ~ GrazeTime + Treatment + 
                            1, data = mbc17, na.action = "na.fail")
)
z

# repeated-measures ANOVA
summary(aov(log1p(MBN_mgkgdrysoil) ~ Treatment * GrazeTime + Block + Error(log1p(MBN_mgkgdrysoil) / GrazeTime), data = mbc.avg))

# paired bonferroni test as post-hoc
with(mbc.avg, pairwise.t.test(log1p(MBC_mgkgdrysoil), Treatment, p.adjust.method = "bonferroni", paired = TRUE))
# this doesn't show any significance
...
After Block 4 is removed, the log-transformed Gaussian GLM shows significance with GrazeTime. However, a repeated-measures ANOVA does not show significance.
```{r}

veg17.avg <- veg17.avg[veg17.avg$Block %in% 1:3,]

x <- step1to3(data = veg17.avg, variable.name = "biomass_kg_plot")
y <- step4to5(data = veg17.avg, variable.name = "biomass_kg_plot")
y
# make sure to change "data" to the dataframe
z <- step6(bestModel = glm(formula = log1p(biomass_kg_plot) ~ GrazeTime + 1, data = veg17.avg, 
                           na.action = "na.fail"))
z

# since log-transfomed meets linear regression assumptions, do ANOVA
summary(aov(log1p(biomass_kg_plot) ~ Treatment*GrazeTime + Block + Error(log1p(biomass_kg_plot) / GrazeTime), data = veg17.avg))
